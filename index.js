...
globalThis.rabbitNumeralRandomizerInterceptor = async function(chat, contextSize, abort, type) {
    try {
        ...
        if (!settings.randomWords?.enabled) {
            ...
            if (words && words.length > 0) {
                const wordStrings = words.map(w => typeof w === 'object' ? w.word : w);
                globalThis.lastRabbitWords = wordStrings;
...
jQuery(async () => {
    ...
    registerMacro('words', (nonce) => {
        try {
            ...
            const count =
                Number.isFinite(rw.wordCount) && rw.wordCount > 0
                    ? rw.wordCount
                    : 3;

            let wordStrings = [];

            // 1) Prefer words generated by the interceptor (API / full logic)
            if (Array.isArray(globalThis.lastRabbitWords) && globalThis.lastRabbitWords.length > 0) {
                wordStrings = globalThis.lastRabbitWords.slice(0, count);
            } else {
                // 2) Fallback: purely local, synchronous random words
                ...
                addToHistory(picked);
                wordStrings = picked.map(w => typeof w === 'object' ? w.word : w);
            }
            ...
            const fallback = getRandomFallbackWords(3);
